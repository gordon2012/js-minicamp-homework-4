...
function multiplyArguments() {
	...
	return arguments.length === 0 ? 0 : Array.from(arguments).reduce(function(acc, val) { return acc * val; }, 1);
}
...
function sumArray(numbers, cb) {
	...
	cb(numbers.reduce(function(acc, val) { return acc + val; }, 0));
}

function forEach(arr, cb) {
	//iterate over arr and pass its values to cb one by one
	//hint: you will be invoking cb multiple times (once for each value in the array)
	arr.forEach(function(e) { cb(e); });
}
...
function map(arr, cb) {
	...
	return arr.map(function(e) { return cb(e); });
